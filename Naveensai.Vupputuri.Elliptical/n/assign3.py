...
def assignment3(x1,y1,x2,y2,a,b) :
    ...
    x3 = 0.0
    y3 = 0.0
    if (((y1*y1 - x1*x1*x1 - a*x1 - b)==0) and ((y2*y2 - x2*x2*x2 - a*x2 - b)==0)) :
        if ((x1 == x2)and(y1== y2)):
    		m =(3 * x1*x1 +a)/(2*y1)
        else :
    		m = (y1-y2)/(x1-x2)
        x3= round(m*m -x1-x2,2)
        y3 = round(m*(x3-x1)+y1,2)
        
        # Annotate the plot with your name using width (w) and height (h) as your reference points.
        an1 = plt.annotate("Naveen sai vupputuri", xy=(-w+2 , h-2), xycoords="data",
                      va="center", ha="center",
        ...
        print (" The point does not lie on the curve")
